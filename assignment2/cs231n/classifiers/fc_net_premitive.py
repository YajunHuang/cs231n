...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        linear = None
        activation = X
        activations = [activation]        # store the activation values for input layer and each hidden layers
        for l in xrange(1, self.num_layers + 1):
            W = self.params['W' + str(l)]
            b = self.params['b' + str(l)]
            linear = np.dot(activation, W) + b
            if l != self.num_layers:
                activation = np.maximum(linear, 0)
        ...
        # scores layer grads
        scores_grad = np.copy(softmax_scores)
        scores_grad[np.arange(N), y] -= 1

        # hidden layers
        grad = scores_grad
        for l in xrange(self.num_layers):
            # using the (l-1)th layer activation and the the lth layer grad to compute the lth layer dW
            dW = np.dot(activations[self.num_layers - l - 1].T, grad) / N
            dW += self.reg * self.params['W' + str(self.num_layers - l)]
            db = np.sum(grad, axis=0) / N
            grads['W' + str(self.num_layers - l)] = dW
            grads['b' + str(self.num_layers - l)] = db

            # update grad to the last non-linear(ReLu) layer
            grad = np.dot(grad, self.params['W' + str(self.num_layers - l)].T)
            # update grad to the last linear layer
            grad[activations[self.num_layers - l - 1] == 0] = 0
...
if __name__ == '__main__':
    fcnet = FullyConnectedNet([15, 20, 15])
